...
<description>
Skills act as additions to models, so effectiveness depends on the underlying model. What works for a frontier model might need more detail for a smaller model.
</description>
...
- Smaller models get a complete working example
- Balanced models get a clear default with an escape hatch
- Frontier models get enough context without over-explanation

**Too minimal for smaller models**:
```xml
<quick_start>
Use pdfplumber for text extraction.
</quick_start>
...
**Too verbose for frontier models**:
```xml
<quick_start>
...
<description>
SKILL.md serves as an overview. Knowledge files contain details. The agent loads knowledge files only when needed.
</description>
...
<model_testing>
Test with all target models. Balance detail level across smaller, balanced, and frontier models.
</model_testing>
